...
import { getBondCalculator1 } from "src/helpers/BondCalculator";
...
function BondPurchase({ bond, slippage, recipientAddress }) {
  ...
  const setMax = async () => {
    let maxQ;
    const balance = parseFloat(bond.balance);
    if (!bond.isLP) {
      maxQ = (bond.maxBondPrice - 0.00001) * bond.bondPrice;
    } else {
      const bondCalcContract = getBondCalculator1(chainID, provider);
      const valuation = await bondCalcContract.valuation(bond.getAddressForReserve(chainID), parseInt(balance * 10 ** 18).toString());
      const markdown = await bondCalcContract.markdown(bond.getAddressForReserve(chainID));
      const lpValue = valuation.mul(markdown).div("0x" + (10 ** 15).toString(16));
      const totalCost = bond.maxBondPrice * bond.bondPrice;
      maxQ = bond.balance * (totalCost / lpValue.toNumber());
      maxQ = (maxQ * 0.9999).toFixed(14);
    }
  ...
  const bondDetailsDebounce = useDebounce(quantity, 100);
              ...
              <div className="help-text">
                <em>
                  <Typography variant="body1" align="center" color="textSecondary">
                    First time bonding <b>{displayName}</b>? <br /> Please approve Atreides Dao to use your{" "}
                    <b>{displayName}</b> for bonding.
                  </Typography>
                </em>
              </div>
            ) : (
              <FormControl className="spice-input" variant="outlined" color="primary" fullWidth>
      ...
      <Slide direction="left" in={true} mountOnEnter unmountOnExit {...{ timeout: 533 }}>
        <Box className="bond-data">
          ...
          <div className="data-row">
            ...
            <Typography>
              {isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.debtRatio, 2)}%`}
            </Typography>
          </div>
