...
export const calcBondDetails = createAsyncThunk(
  ...
  async ({ bond, value, provider, networkID }: ICalcBondDetailsAsyncThunk, { dispatch }): Promise<IBondDetails> => {
    ...
    const bondCalcContract = getBondCalculator1(networkID, provider);
    const terms = await bondContract.terms();
    const maxBondPrice = await bondContract.maxPayout();
    let debtRatio = (await bondContract.standardizedDebtRatio());
    // TODO Find a way to fix this.
    if (bond.name == 'usdt') {
      debtRatio = debtRatio / (10 ** 7);
    } else if (bond.name == 'spice_usdt_lp') {
      debtRatio = debtRatio / (10 ** 4);
    }
    ...
    if (bond.decimals) {
      ...
      bondDiscount = bondDiscount / Math.pow(10, bond.decimals * 2) - 1;
    } else {
