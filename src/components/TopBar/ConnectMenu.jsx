...
function ConnectMenu({ theme }) {
  ...
  if (chainID !== 42262) {
    buttonText = "Switch to Emerald Paratime";
  ...
  const id = open ? "spice-popper-pending" : undefined;

  const primaryColor = "#F8CC82";
  const buttonStyles =
    "pending-txn-container" + (isHovering && pendingTransactions.length > 0 ? " hovered-button" : "");

  const getEtherscanUrl = txnHash => {
    return "https://explorer.emerald.oasis.dev/tx/" + txnHash;
  };
